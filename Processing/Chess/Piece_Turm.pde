class Turm {
  ...
  void showText() {
    if (!moving) {
      if (team == 'A') {
        image(TurmS, pos.x, pos.y);
      } else {
        image(TurmW, pos.x, pos.y);
      }
    } else {
      if (team == 'A') {
        image(TurmS, mousePos.x, mousePos.y);
      } else {
        image(TurmW, mousePos.x, mousePos.y);
      }
    }
  }
  ...
  void allPossibleMoves(boolean active, boolean team, boolean AddToPath) {
    updatePossibleMoves(false, true, 10, new PVector(0, 0), team, false, false, 0, posStatic, false, active, AddToPath);
  }
