...
class Root:
	...
	def getEncoding(self):
		...
		if self.letter:
			Encoding[self.letter] = self.traverseUp()
		if self.childB:
...
for d, v in sorted(Encoding.items(), key=lambda item: (text.count(str(item[0]))), reverse=True):
	print(str(text.count(d)) + "['" + str(d) + "':" + str(Encoding.get(d)) + "]  ", end='')
...
print("Original:    ", text)
print("\n\nCompressed to:", str(round(lenCompressed / lenOriginal * 100, 3)) + "%  Comp/Orig : ", lenCompressed, "/", lenOriginal)
